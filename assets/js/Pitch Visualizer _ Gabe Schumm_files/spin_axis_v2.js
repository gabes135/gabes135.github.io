...
function initBabylon() {
    ...
    // Camera
    // Look along +Y, view is X-Z plane
    // const camera = new BABYLON.ArcRotateCamera(
    //     "camera",
    //     -Math.PI / 2,    // alpha: rotate around Y to point along +Y
    //     -Math.PI / 2,     // beta: vertical angle so camera is level with X-Z plane
...
function updateAnimation(omega_hat) {
    ...
    BABYLON.MeshBuilder.CreateTube(null, { path: points, instance: seamMesh }); // update in place
    ...
    BABYLON.MeshBuilder.CreateTube(null, { path: points, instance: axisMesh }); // update in place
    arrowMesh.position = points[1].clone();
    const axis_path = new BABYLON.Path3D(points);
...
function animatePitch(omega_hat, omega_mag) {
    ...
    theta = ((omega_mag * 0.10472) / 60) * 0.03;

...
const scale_factor = .05
const m_to_feet = (1/0.3048) * scale_factor
...
function plot_traj(row, f_L=1) {
    ...
    // Feet
    pitch_state.scale_factor = scale_factor
    for (let i = 1; i <= 3; i++) {
        if (data[i]) {
    ...
    pitch_state.data_0 = data
    pitch_state.data_nospin = data_nospin
   ...
   const mound =  makeFlatCircle(0, 60.5* scale_factor, 9 * scale_factor,
    ...
    x_cam = (hand === 'R') ? -0.1* scale_factor : 0.1* scale_factor;
    ...
    if (catcher_view) { 
            ...
            camera_settings =  {
            up: { x: 0, y: 0, z: 1 },
            center: { x: 0, y: 0, z: 0* scale_factor },
            eye: {x: -x_cam, y:  20*scale_factor,z: .2},
            projection: {
            type: 'perspective'
          }
        }
    ...
    // 
    return pitch_state
    // [pitch_state.C_T, pitch_state.C_L,

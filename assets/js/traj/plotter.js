...
let pitch, pitch_nospin, start, end, end_nospin
let meshes = []
...
function reset_camera(){
    ...
    const anims = Object.entries(targets).map(([prop, target]) => {
...
function generate_trajectory(pitch_state){
    ...
    let data = calcOdeRK4(solver, 1000)
...
function recalcTraj(row, f_L, omega_hat){
    ...
    let pitch = plot_scene.getMeshByName("pitch"); 
    let end = plot_scene.getMeshByName("end"); 

    if (!plot_scene.getMeshByName("pitch0")){
        let end0 = end.clone("end0")
        end0.material = end0.material.clone("end0Mat");
        end0.material.alpha = .33

        let vertices = makeVector3List(oldData[1], oldData[2], oldData[3])
        let pitch0 = makeTube(vertices, [0, 0, 0], .33, 'pitch0', radius = .05) 
    ...
    solver_prime = new ODEsolver(pitch_state.derivs.bind(pitch_state), pitch_state.initial_conditions, 0, 1)
    data = calcOdeRK4(solver_prime, 1000)
    ...
    let vertices = makeVector3List(data[1], data[2], data[3])
    ...
    end.position.set(data[1].at(-2), data[2].at(-2), data[3].at(-2));
...
function plot_traj(row, noSpin=true) {
    ...
    if (init) {
        ...
        plot_scene.getMeshByName("end").position.set(data[1].at(-2), data[2].at(-2), data[3].at(-2));
        ...
        console.log(`My final pos: (${(data[1].at(-2)/scale).toFixed(3)}, ${(data[3].at(-2)/scale).toFixed(3)})`)
        ...
        let end = makeMarker([data[1].at(-2), data[2].at(-2), data[3].at(-2)], [0, 0, 1], 1, 'end', diameter = ball_diam)
        ...
        if (noSpin){
            ...
            let end_nospin = makeMarker([data_nospin[1].at(-2), data_nospin[2].at(-2), data_nospin[3].at(-2)], [0, 0, 1], .33, 'end_nospin', diameter = ball_diam)
